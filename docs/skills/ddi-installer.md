...
8. The wrapper assigns the new `/var` partition a per-install GPT UUID and
   installs a global UKI addon at `ESP/loader/addons/bluefin-var.addon.efi`.
   Its `systemd.mount-extra=` kernel option mounts that `PARTUUID` on `/var`
   across UKI updates, even if another disk has a `var` partition label.
